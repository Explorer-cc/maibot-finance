...
它还不是已完成的 M1 或 M2：M1 的视觉/embedding 凭据确认与安全图片验证尚未执行，但独立阶段配置、启动参数和预检已实现；M2 的知识资料、知识导入、硬限流和告警也尚未执行。M2 不设置人工审核或一周主观观察门槛。
...
| 配置兼容性 | M0/M2 已通过源码模型验证；M1 已通过离线预检 | M0 与 `m2` 渲染配置均已通过 MaiBot 1.0.12 的 Pydantic 配置模型与本地预检；本轮新增独立 M1 配置、启动参数和预检，并以虚构凭据通过 Compose 渲染。真实 M1 模型调用仍待运营者填写实际 M1 值后验证。 |
...
| M0/M2 配置加载 | 已通过容器实际模型校验 | M0 加载结果：A_Memorix `false`、遥测 `false`、日志 `WARNING`、1 个模型；M2：A_Memorix `true`、遥测 `false`、日志 `WARNING`、4 个模型。独立 M1 使用虚构凭据通过结构/Compose 预检，包含 4 个模型且 A_Memorix 保持关闭。 |
...
4. `m1` 生成器现已将 Qwen-VL、豆包视觉和豆包 embedding 与 M2 的 A_Memorix 集成分离：M1 关闭 A_Memorix、检索工具与人物画像注入，仅用于模型可用性和安全图片验证。变更 embedding 模型或维度必须重建 M2 索引。
...
- 确认 Qwen-VL、豆包视觉和豆包 embedding 凭据、模型 ID 与 embedding 实际维度；以一张预先约定的非敏感图片验证视觉顺序回退。
- 确保 M1 不导入金融资料、不启用金融检索，也不为 embedding 测试自动写入群摘要或人物事实。
