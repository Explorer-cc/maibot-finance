...
它还不是已完成的 M1 或 M2：M1 的 Qwen-VL 凭据确认与安全图片验证尚未执行，但独立阶段配置、启动参数和预检已实现；M2 再接入 Qwen embedding。M2 的知识资料、知识导入、硬限流和告警也尚未执行。M2 不设置人工审核或一周主观观察门槛。
...
| 模型配置加载 | M0 已通过容器实际模型校验；当前 M1/M2 待验证 | M0 加载结果：A_Memorix `false`、遥测 `false`、日志 `WARNING`、1 个模型。q84 后的 M1 静态渲染为 DeepSeek + Qwen-VL（A_Memorix 关闭）；M2 为 DeepSeek + Qwen-VL + Qwen embedding（A_Memorix 启用）。两者均尚未做容器加载验证。 |
...
4. `m1` 生成器只加载 Qwen-VL，并关闭 A_Memorix、检索工具、人物画像注入与 embedding，仅用于模型可用性和安全图片验证。M2 再加载 Qwen embedding 并验证实际维度；变更 embedding 模型或维度必须重建 M2 索引。
...
- 确认 Qwen-VL 凭据和模型 ID；以一张预先约定的非敏感图片验证视觉解析与失败降级。
- 确保 M1 不导入金融资料、不启用金融检索、embedding 或自动记忆写回。
